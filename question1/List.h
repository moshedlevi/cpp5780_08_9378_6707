...
class List {
    ...
    public:
        ...
        ~Node();
        // data areas
        T   value() const; // getter only
        Node* next() const;  // getter
        void  next(Node*);   // setter
    }; //end of class Node

    template <typename T,typename R>
    class Iterator : ForwardIterator<T,R> {
        using ValueType = R;
        using Pointer = T*;
        using Reference = R&;
    ...
    public:
        Iterator(Pointer p) : ForwardIterator<T, R>(p) {}
    ...
    auto begin() { return Iterator<List::Node, T>(head);}
    auto end() { return Iterator<List::Node, T>(nullptr); }
...
template <typename T>
T List<T>::Node::value() const { return _value; }
